#!/usr/bin/env python3
"""Smoke test script for OpenAI LLM Provider integration in AI Committee.

Flow:
  OpenAI -> Agent (Architect) -> Structured Output -> Pydantic (ArchitectProposal)
...
def check_api_key() -> str:
    """Verify presence of OPENAI_API_KEY."""
    key = os.getenv("OPENAI_API_KEY")
    if not key or not key.strip():
        print("=" * 70)
        print("AI COMMITTEE — OPENAI SMOKE TEST")
        print("=" * 70)
        print("[ERRO] Variável de ambiente OPENAI_API_KEY não encontrada.\n")
        print("Para executar o smoke test com o modelo real da OpenAI, configure a chave:")
        print("    export OPENAI_API_KEY=\"sua-chave-de-api-aqui\"")
        print("    PYTHONPATH=. .venv/bin/python scripts/smoke_test_openai.py\n")
        print("Opcionalmente, você também pode configurar:")
        print("    export OPENAI_MODEL=\"gpt-4o\"  (ou gpt-4o-mini, o3-mini, etc.)")
        print("    export OPENAI_TIMEOUT_SECONDS=\"60.0\"")
        print("=" * 70)
        sys.exit(1)
    return key.strip()
...
async def run_smoke_test() -> None:
    ...
    print(f"Timeout:     {config.openai_timeout_seconds}s")
    ...
    input_context = {
        "session_id": "smoke-test-openai-session",
    ...
    print(f"[3/4] Solicitando geração estruturada à OpenAI ({config.openai_model})...")
    try:
    ...
    print("=" * 70)
    print("SMOKE TEST OPENAI CONCLUÍDO COM SUCESSO! (Zero persistência no Event Store)")
    print("=" * 70)
