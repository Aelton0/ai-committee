"""Opt-in live integration test for Google Gemini LLM Provider.
...
@pytest.mark.skipif(
    ...
    reason="Live integration tests require RUN_LIVE_LLM_TESTS=1 and GEMINI_API_KEY to be set.",
)
def test_live_gemini_architect_proposal_generation() -> None:
    """Live call to Google Gemini requesting an ArchitectProposal artifact."""
    async def _test():
